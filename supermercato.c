...
config_t* ps_f(char* fname){
    ...
    while(fgets(buf, BUF_SIZE, fd) != NULL){
        ...
        buf++;
        switch(i){
            case 0: { K = cfg -> K = atoi(buf); fprintf(stdout, "K=%d\n", cfg -> K);}
                break;
...
int main(int argc, char* argv[]){
    //if(cliente_log == NULL){ perror("fopen"); exit(EXIT_FAILURE);}
    //srand(time(NULL));
    if(argc != 2){
    ...
    client_args_t      *client_args        =  malloc(sizeof(client_args_t)*C);
    ...
    if(!desks || !thread_cassieri || !client_args || !cassiere_args || !thread_clienti)                    
    ...
    for(int i = 0; i < K; ++i){
        ...
        /** @todo implementare prima apertura casse con file di config, poi con direttore */  
        desks[i].desk_open = 1;
        ql -> open_desks_n++;
        pthread_mutex_init(&desks[i].mutex_cassa, NULL);
        pthread_cond_init(&desks[i].cond_cassa, NULL);
    }
    ...
    for(int i = 0; i < K; ++i){
        /** @todo leggere da file di configurazione del progetto. */
    ...
    //creo i thread clienti, faccio entrare MAXCLIENTI assieme -> produttori rispetto alle casse
    for(int i = 0; i < C; ++i){
    ...
    while(!sighup){
        ...
        //finchè il supermercato è pieno aspettac
